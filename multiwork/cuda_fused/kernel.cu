...
__device__ void mult_gpu(int *A, int *B, int *C, int thread){
  int tid = threadIdx.x;
  int i, k;
  int sum = 0;
  int size = thread;
...
void mult(int *A, int *B, int *C, int thread){
  int i, j, k;
  int size = thread;
  int sum = 0;
  for(j = 0; j < thread; j++)
...
__device__ void get_pixel(int *count, float index, int thread)
{
  ...
  if(tid < thread){
    for(i = 0; i < (size*size/thread); i++){
      x = ( ( float ) (     (i*thread+tid)%size     ) * (x_max + index)
          + ( float ) ( size - ((i*thread+tid)%size) - 1 ) * (x_min + index) )
          / ( float ) ( size     - 1 );

      y = ( ( float ) (     (i*thread+tid)/size     ) * (y_max + index)
          + ( float ) ( size - ((i*thread+tid)/size) - 1 ) * (y_min + index) )
          / ( float ) ( size     - 1 );
...
__device__ void FBCore(float *r, float *H, float *Vect_H, float *Vect_Dn, 
			float *Vect_Up, float *Vect_F, float *F, int size, int thread){
  int tid = threadIdx.x;
  int j, k;
...
__device__ void des_encrypt_dev( uint32 *esk, uint32 *dsk, uint8 *input, uint8 *output, int len, int thread)
{
...
__global__ void d_fused_kernel(int *d_A, int *d_B, int *d_C, int *count, float *index,
                 float *r, float *H, float *Vect_H, float *Vect_Dn,
                 float *Vect_Up, float *Vect_F, float *F,
                 uint32 *esk, uint32 *dsk, uint8 *input, uint8 *output,
                 int *thread, int *num_size, int idx){

    int i = blockIdx.x;
    switch(i/SUB_NUM){
#if 1
      case 0:
        mult_gpu(d_A + num_size[i],
                 d_B + num_size[i],
                 d_C + num_size[i],
                 thread[idx*TK_NUM+i]);
        break;
      case 1:
	 get_pixel(count + num_size[i],
                         index[idx*TK_NUM+i],
                         thread[idx*TK_NUM+i]);
        break;
#endif
      case 2:
         FBCore(r + num_size[i], 
                       ...
                       Vect_F + num_size[i], 
                       F + N_col*(i%SUB_NUM), 
		       thread[idx*TK_NUM+i]*thread[idx*TK_NUM+i], 
		       thread[idx*TK_NUM+i]);
        break;

      case 3:
	  des_encrypt_dev( esk, dsk, 
                          input + num_size[i],                     
                          output + num_size[i],                                       
                          (thread[idx*TK_NUM+i]*thread[idx*TK_NUM+i])/8, 
			  thread[idx*TK_NUM+i]);
        break;

   }
}
void fused_kernel(int *h_A, int *h_B, int *h_C, int *count, float *index,
                 float *r, float *H, float *Vect_H, float *Vect_Dn,
                 float *Vect_Up, float *Vect_F, float *F,
                 uint32 *esk, uint32 *dsk, uint8 *input, uint8 *output,
		 int *thread, int *num_size, int idx){
