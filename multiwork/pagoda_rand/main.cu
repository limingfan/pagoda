...
int main(){
  ...
  //mem copy
    for(i = 0; i < TK_NUM; i++){
    checkCudaErrors(cudaMemcpyAsync(d_A[i], h_A[i], num_size[i] * num_size[i]*sizeof(int), cudaMemcpyHostToDevice, runtime_stream));
    checkCudaErrors(cudaMemcpyAsync(d_B[i], h_B[i], num_size[i] * num_size[i]*sizeof(int), cudaMemcpyHostToDevice, runtime_stream));
  }
  for(i = 0; i < TK_NUM; i++){

    checkCudaErrors(cudaMemcpyAsync(d_r[i], h_r[i], num_size[i+2*TK_NUM] * num_size[i+2*TK_NUM]*sizeof(float), cudaMemcpyHostToDevice, runtime_stream));
    checkCudaErrors(cudaMemcpyAsync(d_Vect_Up[i], h_Vect_Up[i], num_size[i+2*TK_NUM] * num_size[i+2*TK_NUM]*sizeof(float), cudaMemcpyHostToDevice, runtime_stream));
    checkCudaErrors(cudaMemcpyAsync(d_Vect_F[i], h_Vect_F[i], num_size[i+2*TK_NUM] * num_size[i+2*TK_NUM]*sizeof(float), cudaMemcpyHostToDevice, runtime_stream));
    checkCudaErrors(cudaMemcpyAsync(d_Vect_H[i], h_Vect_H[i], num_size[i+2*TK_NUM] * num_size[i+2*TK_NUM]*sizeof(float), cudaMemcpyHostToDevice, runtime_stream));
  ...
  for(i = 0; i < TK_NUM; i++){
    checkCudaErrors(cudaMemcpyAsync(d_packet_in[i], h_packet_in[i], num_size[i+3*TK_NUM] * num_size[i+3*TK_NUM]*sizeof(unsigned char), cudaMemcpyHostToDevice, runtime_stream));
  ...
  for(i = 0; i < task; i++){
    switch(i%4){
      case 0:
        ...
        taskLaunch(10, INT, num_thread[mult_c]*32, INT, 1, INT, 0, INT, 0, INT, 0, INT, d_A[mult_c], 
			INT, d_B[mult_c], INT, d_C[mult_c], INT, num_size[mult_c], INT, num_thread[mult_c]*32);
        mult_c ++;
        break;

      case 1:
        //get_pixel<<<1, TD_NUM, 0, work_stream[mand_c]>>>(d_count[mand_c], &d_task_indx[mand_c]);
        taskLaunch(9, INT, num_thread[mand_c+TK_NUM]*32, INT, 1, INT, 0, INT, 0, INT, 1, INT, d_count[mand_c], INT, &d_task_indx[mand_c], 
			INT, num_size[mand_c+TK_NUM], INT, num_thread[mand_c+TK_NUM]*32);
        mand_c ++;
        break;
#if 1
      case 2:
	...
	//			d_Vect_Dn[filter_c], d_Vect_Up[filter_c], d_Vect_F[filter_c], d_F[filter_c]);
        taskLaunch(14, INT, num_thread[filter_c+2*TK_NUM]*32, INT, 1, INT, 0, INT, 1, INT, 2, FLOAT, d_r[filter_c],
                FLOAT, d_H[filter_c], FLOAT, d_Vect_H[filter_c], FLOAT, d_Vect_Dn[filter_c], FLOAT, d_Vect_Up[filter_c],
                FLOAT, d_Vect_F[filter_c], FLOAT, d_F[filter_c], INT, num_size[filter_c+2*TK_NUM] * num_size[filter_c+2*TK_NUM], 
		INT, num_thread[filter_c+2*TK_NUM]*32);
        filter_c ++;
        break;
      case 3:
        //des_encrypt_dev<<<1, TD_NUM, 0, work_stream[des_c]>>>(d_des_esk, d_des_dsk, d_packet_in[des_c],
        //                                d_packet_out[des_c], LEN/8);
        taskLaunch(11, INT, num_thread[des_c+3*TK_NUM]*32, INT, 1, INT, 0, INT, 0, INT, 3, INT32, d_des_esk, INT32, d_des_esk,
                                CHAR, d_packet_in[des_c], CHAR, d_packet_out[des_c], INT, (num_size[des_c+3*TK_NUM] * num_size[des_c+3*TK_NUM])/8,
				INT, num_thread[des_c+3*TK_NUM]*32);
  ...
  // memory copy back
  for(i = 0; i < TK_NUM; i++){
    checkCudaErrors(cudaMemcpyAsync(h_C[i],d_C[i], num_size[i] * num_size[i]*sizeof(int), cudaMemcpyDeviceToHost, runtime_stream));
  }
  for(i = 0; i < TK_NUM; i++){
    checkCudaErrors(cudaMemcpyAsync(h_count[i], d_count[i], num_size[i+TK_NUM] * num_size[i+TK_NUM]*sizeof(int), cudaMemcpyDeviceToHost, runtime_stream));
  }
  for(i = 0; i < TK_NUM; i++){
    checkCudaErrors(cudaMemcpyAsync(h_Vect_F[i], d_Vect_F[i], num_size[i+2*TK_NUM] * num_size[i+2*TK_NUM]*sizeof(float), cudaMemcpyDeviceToHost, runtime_stream));
  }
  for(i = 0; i < TK_NUM; i++){
    checkCudaErrors(cudaMemcpyAsync(h_packet_out[i], d_packet_out[i], num_size[i+3*TK_NUM] * num_size[i+3*TK_NUM]*sizeof(unsigned char), cudaMemcpyDeviceToHost, runtime_stream));
  }
    ...
    switch(i%4){
      ...
      case 2:
        h_FBCore(h_r[filter_c], h_H[filter_c], h_Vect_H[filter_c], 
			...
			h_Vect_F_host[filter_c], h_F[filter_c], 
			num_size[filter_c+2*TK_NUM] * num_size[filter_c+2*TK_NUM]);
        filter_c ++;
        break;
#if 1
      case 3:
