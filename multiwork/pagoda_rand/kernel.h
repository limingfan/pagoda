...
extern void mult(int *A, int *B, int *C, int size, int thread);
extern void h_FBCore(float *r, float *H, float *Vect_H, 
			float *Vect_Dn, float *Vect_Up, float *Vect_F, float *F, int size);
extern int des_set_key( uint32 *esk, uint32 *dsk, uint8 key1[8],
                                uint8 key2[8], uint8 key3[8]);
extern void h_get_pixel(int *count, float index, int size);
