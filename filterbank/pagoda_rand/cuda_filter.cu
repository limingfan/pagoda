...
int main(){
	...
	printf("Filterbank Pagoda inputs are generating\n");
  	...
  	for(i = 0; i < N_ch; i++){
    		checkCudaErrors(cudaMemcpyAsync(r_dev[i], r[i], num_size[i]*sizeof(float), cudaMemcpyHostToDevice, runtime_stream));
    		checkCudaErrors(cudaMemcpyAsync(Vect_Up_dev[i], Vect_Up[i], num_size[i]*sizeof(float), cudaMemcpyHostToDevice, runtime_stream));
    		checkCudaErrors(cudaMemcpyAsync(Vect_F_dev[i], Vect_F[i], num_size[i]*sizeof(float), cudaMemcpyHostToDevice, runtime_stream));
    		checkCudaErrors(cudaMemcpyAsync(Vect_H_dev[i], Vect_H[i], num_size[i]*sizeof(float), cudaMemcpyHostToDevice, runtime_stream));
	...
	// task launch
  	...
  	for(i = 0; i < N_ch; i++){
    		taskLaunch(14, INT, num_thread[i]*32, INT, 1, INT, 0, INT, 1, INT, 0, FLOAT, r_dev[i],
			FLOAT, H_dev[i], FLOAT, Vect_H_dev[i], FLOAT, Vect_Dn_dev[i], FLOAT, Vect_Up_dev[i],
			FLOAT, Vect_F_dev[i], FLOAT, F_dev[i], INT, num_thread[i]*32, INT, num_size[i]);
  	}
  	waitAll(N_ch);
  	end_timer = my_timer();
  	...
  	for(i = 0; i < N_ch; i++){
    		checkCudaErrors(cudaMemcpyAsync(Vect_F[i], Vect_F_dev[i], num_size[i]*sizeof(float), cudaMemcpyDeviceToHost, runtime_stream));
 	}
  	checkCudaErrors(cudaStreamSynchronize(runtime_stream));
