...
__global__ void des_encrypt_dev( uint32 *esk, uint32 *dsk, uint8 *input, uint8 *output, int len, int thread)
{
...
int main (int argc, char **argv)
{
	...
	cudaSetDevice(0);
  	...
  	num_thread = (int*)malloc(numpackets*sizeof(int));
  	num_size = (int*)malloc(numpackets*sizeof(int));
  	...
  	for(i = 0; i < numpackets; i++){
      		if(num_size[i] == 32){
        		checkCudaErrors(cudaHostAlloc(&packet_in[i], 2*num_size[i]*num_size[i]*sizeof(unsigned char), cudaHostAllocDefault));
        		checkCudaErrors(cudaMalloc(&packet_in_dev[i], 2*num_size[i]*num_size[i]*sizeof(unsigned char)));
        		checkCudaErrors(cudaHostAlloc(&packet_out[i], 2*num_size[i]*num_size[i]*sizeof(unsigned char), cudaHostAllocDefault));
        		checkCudaErrors(cudaMalloc(&packet_out_dev[i], 2*num_size[i]*num_size[i]*sizeof(unsigned char)));
        		packet_open[i] =  (unsigned char *) malloc (2*num_size[i]*num_size[i]* sizeof(unsigned char));
      		}else{
        		checkCudaErrors(cudaHostAlloc(&packet_in[i], num_size[i]*num_size[i]*sizeof(unsigned char), cudaHostAllocDefault));
        		checkCudaErrors(cudaMalloc(&packet_in_dev[i], num_size[i]*num_size[i]*sizeof(unsigned char)));
        		checkCudaErrors(cudaHostAlloc(&packet_out[i], num_size[i]*num_size[i]*sizeof(unsigned char), cudaHostAllocDefault));
        		checkCudaErrors(cudaMalloc(&packet_out_dev[i], num_size[i]*num_size[i]*sizeof(unsigned char)));
        		packet_open[i] =  (unsigned char *) malloc (num_size[i]*num_size[i]* sizeof(unsigned char));
	...
	printf("DES CUDA baseline inputs are generating\n");
  	...
  	for(i = 0; i < numpackets; i++){
     		if(num_size[i] == 32){
     			checkCudaErrors(cudaMemcpyAsync(packet_in_dev[i], packet_in[i], 
				2*num_size[i]*num_size[i]*sizeof(unsigned char), cudaMemcpyHostToDevice, des_stream[i]));
		}
     		else{
        		checkCudaErrors(cudaMemcpyAsync(packet_in_dev[i], packet_in[i], 
				num_size[i]*num_size[i]*sizeof(unsigned char), cudaMemcpyHostToDevice, des_stream[i]));
		}
  	}
	...
	printf("DES CUDA baseline program is running\n");
  	...
  	for(i = 0; i < numpackets; i++){
        	if(num_size[i] == 32){
	  		des_encrypt_dev<<<1, num_thread[i]*32, 0, des_stream[i]>>>( des_esk_dev, des_esk_dev, packet_in_dev[i], 
					packet_out_dev[i],2* num_size[i]*num_size[i]/8, num_thread[i]*32);
        	}else{
          		des_encrypt_dev<<<1, num_thread[i]*32, 0, des_stream[i]>>>( des_esk_dev, des_esk_dev, packet_in_dev[i],
                                        packet_out_dev[i],num_size[i]*num_size[i]/8, num_thread[i]*32);
        	}
  	...
  	for(i = 0; i < numpackets; i++){
     		if(num_size[i] == 32){
       			checkCudaErrors(cudaMemcpyAsync(packet_out[i], packet_out_dev[i], 
				2*num_size[i]*num_size[i]*sizeof(unsigned char), cudaMemcpyDeviceToHost, des_stream[i]));
		}
     		else{
       			checkCudaErrors(cudaMemcpyAsync(packet_out[i], packet_out_dev[i], 
				num_size[i]*num_size[i]*sizeof(unsigned char), cudaMemcpyDeviceToHost, des_stream[i]));
		}
  	}
  	checkCudaErrors(cudaDeviceSynchronize());
	...
	printf("CPU program running\n");
  	...
  	for(i = 0; i < numpackets; i++){
        	if(num_size[i] == 32)
          		des_encrypt(des_esk, des_dsk, packet_in[i], packet_open[i], 2*num_size[i]*num_size[i]/8);
        	else
          		des_encrypt(des_esk, des_dsk, packet_in[i], packet_open[i], num_size[i]*num_size[i]/8);
