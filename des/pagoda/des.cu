...
int main (int argc, char **argv)
{
	...
	printf("Pagoda DES:#thread:%d, #task:%d\n", TDD_NUM, numpackets);
	...
	//generate packet
  	...
  	// copy data to GPU
  	for(i = 0; i < numpackets; i++){
     		checkCudaErrors(cudaMemcpyAsync(packet_in_dev[i], packet_in[i], packet_length[i]*sizeof(unsigned char), cudaMemcpyHostToDevice, runtime_stream));
  	}
  	...
  	// run des
  	for(i = 0; i < numpackets; i++){
		taskLaunch(11, INT, TDD_NUM, INT, 1, INT, 0, INT, 0, INT, 0, INT32, des_esk_dev, INT32, des_esk_dev, 
				CHAR, packet_in_dev[i], CHAR, packet_out_dev[i], INT, packet_length[i]/8, INT, TDD_NUM);
