...
int main (int argc, char **argv)
{
	...
	cudaSetDevice(0);
  	...
  	for(i = 0; i < numpackets; i++){
      		if(num_size[i] == 32){
        		packet_length[i] = 2*num_size[i]*num_size[i];
      		}else{
        		packet_length[i] = num_size[i]*num_size[i];
      		}
	...
	printf("DES Pagoda inputs are generating\n");
  	...
  	// copy data to GPU
  	for(i = 0; i < numpackets; i++){
     		checkCudaErrors(cudaMemcpyAsync(packet_in_dev[i], packet_in[i], packet_length[i]*sizeof(unsigned char), 
					cudaMemcpyHostToDevice, runtime_stream));
  	}
  		checkCudaErrors(cudaMemcpyAsync(des_esk_dev, des_esk, 96*sizeof(uint32), cudaMemcpyHostToDevice, runtime_stream));
	...
	printf("DES Pagoda program is running\n");
  	...
  	// run des
  	for(i = 0; i < numpackets; i++){
		taskLaunch(11, INT, num_thread[i]*32, INT, 1, INT, 0, INT, 0, INT, 0, INT32, des_esk_dev, INT32, des_esk_dev, 
				CHAR, packet_in_dev[i], CHAR, packet_out_dev[i], INT, packet_length[i]/8, INT, num_thread[i]*32);
