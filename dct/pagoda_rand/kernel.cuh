...
__device__ void d_computeDCT(float *fDst, float *fSrc, int Stride, int size, int thread, int baseTid)
{

    int t = baseTid + (threadIdx.x & 0x1f);
    if(t < thread){
        for(int i = 0; i < ((size/BLOCK_SIZE)*(size/BLOCK_SIZE)/thread); i++)
        {
            //process rows
...
__device__ void d_computeIDCT(float *fDst, float *fSrc, int Stride, int size, int thread, int baseTid)
{
...
void computeDCT(float *fDst, float *fSrc, int Stride, int size, int thread)
{
...
void computeIDCT(float *fDst, float *fSrc, int Stride, int size, int thread)
{
        for(int t = 0; t < thread; t++){
          for(int i = 0; i < ((size/BLOCK_SIZE)*(size/BLOCK_SIZE)/thread); i++)
          {
...
__device__ void d_DCT(float *fSrc, float *fDst, int Stride, int size, int thread, int baseTid){
    d_computeDCT(fDst, fSrc, Stride, size, thread, baseTid);
    //d_computeIDCT(fDst, fSrc, Stride, size, thread, baseTid);
}

void DCT(float *fSrc, float *fDst, int Stride, int size, int thread){
    computeDCT(fDst, fSrc, Stride, size, thread);
