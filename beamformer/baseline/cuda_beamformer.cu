...
int main(int argc, char *argv[]){
	...
	if(argc < 3){
		printf("Error input options:./beam #thread #task");
		exit(1);
	}
	int TD_NUM = atoi(argv[1]);
  	int NUM_CHANNELS = atoi(argv[2]);
	printf("CUDA baseline Beamformer:#thread:%d, #task:%d\n", TD_NUM, NUM_CHANNELS);

  	setenv("CUDA_DEVICE_MAX_CONNECTIONS", "32", 1);

  	cudaStream_t *beam_stream;
  	beam_stream = (cudaStream_t*)malloc(NUM_CHANNELS*sizeof(cudaStream_t));
  	for(i = 0; i < NUM_CHANNELS; i++) checkCudaErrors(cudaStreamCreate(&beam_stream[i]));
	...
	printf("Init data....\n");
  	...
  	for (i = 0; i < NUM_BEAMS; i++)
  	{
    	...
    	BeamFormWeights(i, beam_weights[i], TD_NUM);
  	}
  	for (i = 0; i < NUM_CHANNELS; i++) {
  	...
  	//input transfer
  	for (i = 0; i < NUM_CHANNELS; i++) {
    		checkCudaErrors(cudaMemcpyAsync(inputs_dev[i], inputs[i], 2*NUM_SAMPLES*sizeof(float), cudaMemcpyHostToDevice, beam_stream[i%32]));
    		checkCudaErrors(cudaMemcpyAsync(coarse_dev_weight[i], coarse_fir_data[i].weight, 2*NUM_COARSE_TAPS*sizeof(float), 
			cudaMemcpyHostToDevice, beam_stream[i%32]));
  	...
  	checkCudaErrors(cudaMemcpyAsync(post_num_dec1_dev, post_num_dec1, sizeof(int), cudaMemcpyHostToDevice, beam_stream[0]));
  	checkCudaErrors(cudaMemcpyAsync(post_num_dec2_dev, post_num_dec2, sizeof(int), cudaMemcpyHostToDevice, beam_stream[0]));
  	checkCudaErrors(cudaMemcpyAsync(coarse_ratio_dev, coarse_ratio, sizeof(int), cudaMemcpyHostToDevice, beam_stream[0]));
  	checkCudaErrors(cudaMemcpyAsync(fine_ratio_dev, fine_ratio, sizeof(int), cudaMemcpyHostToDevice, beam_stream[0]));
  	checkCudaErrors(cudaMemcpyAsync(coarse_taps_dev, coarse_taps, sizeof(int), cudaMemcpyHostToDevice, beam_stream[0]));
  	checkCudaErrors(cudaMemcpyAsync(fine_taps_dev, fine_taps, sizeof(int), cudaMemcpyHostToDevice, beam_stream[0]));
  	...
  	//Beam filter
  	for (i = 0; i < NUM_CHANNELS; i++) {
    		BeamFirFilter_dev<<<1, TD_NUM, 0, beam_stream[i]>>>(NUM_COARSE_TAPS,
                     NUM_POST_DEC_1,COARSE_DECIMATION_RATIO,
                     coarse_dev_weight[i], coarse_dev_buffer[i],
                     inputs_dev[i],
                     predec_dev[i], TD_NUM);
  	}
  	...
  	for (i = 0; i < NUM_CHANNELS; i++) {
    		BeamFirFilter_dev<<<1, TD_NUM, 0, beam_stream[i]>>>(NUM_FINE_TAPS,
                     NUM_POST_DEC_2, FINE_DECIMATION_RATIO,
                     fine_dev_weight[i], fine_dev_buffer[i],
                     predec_dev[i],
                     postdec_dev[i], TD_NUM);
  	...
  	for (i = 0; i < NUM_CHANNELS; i++) {
    		checkCudaErrors(cudaMemcpyAsync(postdec[i], postdec_dev[i], 2*NUM_POST_DEC_2*sizeof(float), cudaMemcpyDeviceToHost, beam_stream[i]));
  	}
  	checkCudaErrors(cudaDeviceSynchronize());
...
void BeamFormWeights(int beam, float *weights, int NUM_CHANNELS)
{
  int i;
  for (i = 0; i < NUM_CHANNELS; i++)
  {
                        ...
                        float *in, float *out, int TD_NUM)
{
  ...
  //for(k = 0; k < TD_NUM; k++){
  if(tid < TD_NUM){
    for(j = 0; j < (input_length/TD_NUM); j++){
      float real_curr = 0;
      float imag_curr = 0;
      //modPos = 2*(len - 1 - data->pos);
      modPos = 2*(len - 1 - ((j*TD_NUM+tid) & mask));
      buffer[modPos] = in[(j*TD_NUM+tid) * decimation_ratio * 2 ];
      buffer[modPos+1] = in[(j*TD_NUM+tid) * decimation_ratio * 2 + 1];
      ...
      //data->pos = (data->pos + 1) & mask;
      out[(j*TD_NUM+tid) * 2] = real_curr;
      out[(j*TD_NUM+tid) * 2 + 1] = imag_curr;
    }
  }
}
